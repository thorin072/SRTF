       ...
       while True:
           if self.array:
               ...
               if self.array[PID_executed_index].BurstTime==0:
                   ...
                   self.__UpdateQueue(self.array[:sl],PID_executed_index-1,time_CPU,len(self.array),True)
                   PID_executed=self.__NextProcess(self.array[:sl],key=True);
                   sl+=1
               else:
                    #обновление очереди
                    self.__UpdateQueue(self.array[:sl],PID_executed_index,time_CPU,len(self.array),False)
                    PID_executed=self.__NextProcess(self.array[:sl],key=True);
                    sl+=1
           else:
                return list_pos_srtf 
   
   def __UpdateQueue(self,listProcess,PID_ind,timeCPU,n,delete=False):
       swapPID=-10000
       curUpPID=-10000
